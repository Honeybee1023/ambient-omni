...
#SBATCH --output=/data/scratch/honjar/train_logs/%j_cb_eval.out
#SBATCH --requeue
...
CKPT=$(ls /data/scratch/honjar/train_outputs/*-${DATASET_NAME}-*/network-snapshot-001000.pkl 2>/dev/null | head -1)
if [ -z "$CKPT" ]; then
...
# Step 2: Compute FID against holdout (skip if already done)
if [ -f "$OUTJSON" ]; then
    echo "Metrics already computed, skipping."
else
    if [ ! -f "$REF_STATS" ]; then
        echo "ERROR: Reference stats not found at $REF_STATS"
        echo "Run precompute_celeba_fid_stats.sh first!"
        exit 1
    fi
