#!/usr/bin/env python3
"""Create v2b conditional sweep: B2 fixed at T=0.55, sweep B5.
Also creates B4 T=0.525 rerun dataset.
...
def main():
    print("=== Conditional sweep: B2=0.55 fixed, sweep B5 (proline) ===")
    created = 0
    b5_t_values = [0.0, 0.2, 0.4, 0.45, 0.5, 0.525, 0.6, 0.7, 0.8, 0.9, 0.95]
    for t5 in b5_t_values:
        t_int = round(t5 * 1000)
        if t_int % 10 == 0:
            suffix = f"T{t_int // 10:03d}"
        else:
            suffix = f"T{t_int:04d}"
        name = f"celeba_v2b_cond_b5_{suffix}"
        if create_dataset(name, {2: B2_FIXED_T, 5: t5}):
            created += 1

    # B4 T=0.525 rerun (same config, new dataset dir for clean separation)
    print("\n=== B4 T=0.525 rerun ===")
    if create_dataset("celeba_v2b_cond_b4_T0525_r2", {2: B2_FIXED_T, 4: 0.525}):
        created += 1
