...
DATASETS=(celeba_v2b_cond_b2_T099 celeba_v2b_cond_b2_T100)

for NAME in "${DATASETS[@]}"; do
  echo ""; echo "==============================="
  echo "GPU 3: $NAME ($(date))"
  echo "==============================="
  export MASTER_PORT=10004
  ${AMBIENT_BASE}/miniconda3/envs/ambient/bin/python -m torch.distributed.run --standalone --nproc_per_node=1 train.py \
  ...
  if [ ! -d "$OUTDIR" ] || [ $(ls "$OUTDIR"/*.png 2>/dev/null | wc -l) -lt 5000 ]; then
    ...
    ${AMBIENT_BASE}/miniconda3/envs/ambient/bin/python -m torch.distributed.run --standalone --nproc_per_node=1 generate.py \
      --network=$CKPT --outdir=$OUTDIR --seeds=0-4999 --batch=64
  fi
  if [ ! -f "$MIND_JSON" ]; then
    ${AMBIENT_BASE}/miniconda3/envs/ambient/bin/python eval_mind.py --gen_path=$OUTDIR --ref_path=$HOLDOUT \
      --ref_cache=$MIND_CACHE --out_path=$MIND_JSON
  fi
  if [ ! -f "$VALLOSS_JSON" ]; then
    ${AMBIENT_BASE}/miniconda3/envs/ambient/bin/python eval_val_loss.py --checkpoint=$CKPT \
