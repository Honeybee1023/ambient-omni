...
class AmbientEDMLoss:
    ...
    def __call__(
        ...
        **model_kwargs,
        ...
        noise_tn_to_t = jax.random.normal(noise_rng, x_tn.shape, dtype=x_tn.dtype) * jnp.sqrt(jnp.square(sigma_t) - jnp.square(sigma_tn))
...
class AmbientEDMCLSLoss:
    ...
    def __call__(
        ...
        **model_kwargs,
        ...
        sigma_t = jnp.asarray(sigma_t, dtype=jnp.float32).reshape(-1, 1, 1, 1)
        noise_rng, dropout_rng = jax.random.split(rng)
        x_t = x0 + jax.random.normal(noise_rng, x0.shape, dtype=x0.dtype) * sigma_t
        output = apply_fn(
            variables,
            x_t,
            sigma_t.squeeze(),
